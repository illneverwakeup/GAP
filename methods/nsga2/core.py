...
LOGGER = logging.getLogger("gap_nsga2")
...
class NSGA2Experiment:
    """Классический NSGA-II для многокритериальной GAP-постановки.
        ...
        for repeat in range(1, self.config.ga.repeats + 1):
            ...
            for pop_size, p_cross, p_mut, max_gen in product(
                ...
                for generation in range(max_gen):
                    parent_pool = self.nsga2_parent_selection(population, pop_size)
                    offspring = algorithms.varAnd(parent_pool, self.toolbox, p_cross, p_mut)
                ...
                first_front = tools.sortNondominated(population, k=len(population), first_front_only=True)[0]
                final_ranges = self.calculate_population_ranges(population)
                elapsed_time = time.time() - start_time
                result_rows = self.collect_result_rows(first_front, repeat, pop_size, p_cross, p_mut, max_gen, elapsed_time, final_ranges)
...
def run_experiment(config: ExperimentConfig) -> str:
    experiment = NSGA2Experiment(config)
