...
def validate_problem(problem: ProblemData, require_each_employee_used: bool = True) -> None:
    ...
    n, m = c_matrix.shape
    if r_matrix.shape != (n, m):
        raise ValueError(f"Матрица ресурсов R должна иметь размер {(n, m)}, получено {r_matrix.shape}.")
    if b_list.shape[0] != n:
        raise ValueError(f"Длина b должна быть {n}, получено {b_list.shape[0]}.")
    if require_each_employee_used and m < n:
        raise ValueError("Невозможно назначить каждому сотруднику хотя бы одну задачу: задач меньше, чем сотрудников.")
    ...
    for task_idx in range(m):
        if not np.any(r_matrix[:, task_idx] <= b_list + 1e-9):
            raise ValueError(f"Задачу {task_idx + 1} невозможно назначить ни одному сотруднику.")
...
def read_assignment_excel(
    ...
    try:
        ...
        raise ValueError("В A1 должно быть число сотрудников, в A3 — число задач.") from exc
    ...
    try:
        c_matrix = df.iloc[c_start:c_start + n, 0:m].astype(float).to_numpy()
        r_matrix = df.iloc[r_start:r_start + n, 0:m].astype(float).to_numpy()
        b_list = df.iloc[b_start:b_start + n, 0].astype(float).to_numpy()
    except Exception as exc:
        raise ValueError("Не удалось извлечь C, R и b из Excel. Проверьте формат файла.") from exc

    problem = ProblemData(n, m, c_matrix, r_matrix, b_list)
